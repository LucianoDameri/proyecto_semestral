...
SPA React 18 + Vite 5 + Tailwind 3 para la gestion de **Ventas** y **Despachos** del proyecto Innovatech.
...
+- src/
   +- api/config.js       # URLs y headers centralizados (VENTAS_URL, DESPACHOS_URL)
   +- componentes/CrudAdmin/
      +- TableCompras.jsx       # llama VENTAS_URL
      +- TableDespachos.jsx     # llama DESPACHOS_URL
      +- FormDespacho.jsx       # PUT venta + POST despacho
      +- FormCierreDespacho.jsx # PUT despacho
```

## Variables de entorno

### Build-time (Vite, leidas por `import.meta.env`)
...
En `docker compose` -> nombres DNS de la red. En EC2 -> IP privada del backend (ej. `10.0.2.5:8080`).
...
Vite proxy redirige `/api/v1/ventas` -> `localhost:8080` y `/api/v1/despachos` -> `localhost:8081`
(requiere los backends levantados, ej. con `docker compose up ventas despachos mysql`).
...
- **Usuario no root**: la imagen base corre como `nginx` (UID 101) y escucha en 8080 sin `CAP_NET_BIND_SERVICE`.
